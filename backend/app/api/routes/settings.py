...
def _get_or_create(db: Session) -> SiteSettings:
    ...
    if not row:
        ...
        db.commit()
        db.refresh(row)
    elif not row.terms_content:
        # Backfill for a settings row that already existed before terms_content
        # was added (e.g. the current dev/prod DB) — never overwrites real content.
        row.terms_content = _DEFAULT_TERMS
        db.commit()
        db.refresh(row)
    return row
